...
// Lives in a (home) route group so it wraps only /v2 and /v2/[project] —
// /v2/about is a sibling outside the group and must not inherit this UI.
//
...
function projectFromPathname(pathname: string): Project | null {
  ...
  const href = pathname.replace(/^\/v2/, "")
...
export default function V2Layout({ children }: { children?: React.ReactNode }) {
  ...
  const openCaseStudy = useCallback((project: Project) => {
    ...
    window.history.pushState(null, "", `/v2${project.href}`)
  }, [])
